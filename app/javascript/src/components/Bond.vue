<template>
  <div :class="'m-2 border-2 border-' + border() +'-500 p-1 tooltip bg-' + bond.nation.value">
    <svg xmlns="http://www.w3.org/2000/svg" width="30" height="20">
      <Flag :nation="bond.nation.value" width="30" />
    </svg>
    {{ bond.number }}:{{ bond.cost }}
    <div v-if="!!tradedBond" class="tooltip-text border border-gray-500 p-1 rounded mt-3">
...
export default {
  name: "Bond",
  props: { bond: Object, tradedBond: Object },
  components: { Flag },
  methods: {
    border: function() {
      if (!!this.tradedBond) {
