<template>
  <div class="p-4">
    ...
    <div class="flex border-b border-black mt-2">
      ...
      <div class="w-1/3 sm:w-1/5"><b>Current Player</b></div>
      <div class="w-1/3 sm:w-1/5"><b>Last Move At</b></div>
      <div class="hidden sm:w-1/5 sm:inline-block"></div>
    </div>
    <a
      href="/games/new"
    ...
    <div v-for="game of games" v-bind:key="game.id">
      <router-link :to="{ path: '/game/' + game.id }" class="flex justify-between items-center hover:bg-gray-200 py-2">
        ...
        <div class="w-1/3 sm:w-1/5">{{ currentPlayer(game) }}</div>
        <div class="w-1/3 sm:w-1/5">{{ toTime(game.lastMoveAt) }}</div>
        <div class="hidden sm:w-1/5 sm:inline-block" v-if="!game.winner">
        ...
        <div v-else class="hidden sm:w-1/5 sm:inline-block"></div>
      </router-link>
    </div>
